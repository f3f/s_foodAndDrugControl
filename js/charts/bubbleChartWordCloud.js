...
(function () {
    ...
    var render_value = function (/* value */ v) {
        ...
        // 数据
        if ($ = v['data']) for (value['data'] = $, a = value['option']['series'][0]['nodes'], i = 0; b = $[i]; i++) a.push({

            // 标签
            label: {
                normal: {show: true, formatter: b.name, textStyle: {fontSize: (c = (b.value || 0) * 8 + 12)}},
                emphasis: {show: true, formatter: b.name, textStyle: {fontSize: c}}
            },
            ...
            symbolSize: [(b.name.length * 23), ((b.value || 0) * 15 + 30)],
